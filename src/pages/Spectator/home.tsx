        ...
        if (Array.isArray(jList)) {
          ...
          const detailedJockeys = await Promise.all(
            topJockeys.map(async (j: any) => {
              ...
              try {
                const countRes =
                  await jockeyService.getCompletedRacesCount(jockeyId);
                completedRaces =
                  countRes?.data !== undefined ? countRes.data : countRes || 0;
              } catch (err) {
                console.error(
                  `Failed to fetch completed races count for jockey ${jockeyId}`,
                  err,
                );
              }
              return {
    ...
    <div className="max-w-5xl mx-auto px-6 py-12 space-y-14">
      ...
      <div className="space-y-3">
        <h1 className="text-3xl font-bold text-gray-900">
          Horse Racing Tournament Portal
        </h1>
        <p className="text-gray-500 text-base max-w-xl">
          Track tournaments, race schedules, and jockey performance across the
          entire system.
        </p>
        <div className="flex gap-3 pt-1">
          <Link href="/spectator/tournaments">
            <Button size="sm" data-testid="hero-btn-tournaments">
              View Tournaments
            </Button>
          </Link>
          <Link href="/spectator/races">
            <Button size="sm" variant="outline" data-testid="hero-btn-races">
              Race Schedule
            </Button>
          </Link>
        </div>
                  ...
                  <div className="flex items-center gap-4">
                    <Link href={`/spectator/tournaments/${t.id}`}>
                      <button
                        ...
                        data-testid={`btn-tournament-${t.id}`}
                      >
                        Details
                      </button>
                    </Link>
              ...
              {races.map((r) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {r.tournament} · {r.date} · {r.distance}
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
