...
export default function AdminDashboard({ onNavigate }) {
  ...
  const fetchTournaments = async () => {
    try {
      const res = await fetch(
        "http://localhost:8080/api/v1/tournaments/dashboard",
      );
  ...
  useEffect(() => {
    if (newRaceDate && newRaceStartTime && newRaceEndTime) {
      const fetchReferees = async () => {
        ...
        try {
          const res = await fetch(
            `http://localhost:8080/api/v1/referees?date=${newRaceDate}&startTime=${newRaceStartTime}&endTime=${newRaceEndTime}`,
          );
          if (res.ok) {
            const json = await res.json();
  ...
  useEffect(() => {
    if (
      ...
      const fetchReferees = async () => {
        ...
        try {
          const res = await fetch(
            `http://localhost:8080/api/v1/referees?date=${editingRace.date}&startTime=${editingRace.startTime}&endTime=${editingRace.endTime}&excludeRaceId=${editingRace.id}`,
          );
          if (res.ok) {
            const json = await res.json();
            if (json.data && Array.isArray(json.data)) {
  ...
  const handleAddRace = (e) => {
    ...
    const payload = {
      ...
      date: newRaceDate,
      start_time:
        newRaceStartTime.length === 5
          ? `${newRaceStartTime}:00`
          : newRaceStartTime,
      end_time:
        newRaceEndTime.length === 5 ? `${newRaceEndTime}:00` : newRaceEndTime,
    ...
    fetch("http://localhost:8080/api/v1/races", {
      ...
      .then(async (res) => {
        ...
        if (res.ok) {
          showNotification(
            "success",
            json.message || "Thêm mới cuộc đua thành công!",
          );
          fetchRaces(selectedId);
    ...
    fetch(`http://localhost:8080/api/v1/races/${editingRaceId}`, {
      ...
      .then(async (res) => {
        ...
        if (res.ok) {
          showNotification(
            "success",
            json.message || "Cập nhật cuộc đua thành công!",
          );
          fetchRaces(selectedId);
          ...
          showNotification(
            "error",
            json.message || "Cập nhật cuộc đua thất bại!",
          );
        }
      })
      .catch((err) => {
  ...
  return (
    <div
      className="bg-surface-container font-body-sm text-on-surface antialiased flex flex-col lg:flex-row min-h-screen"
      style={{ width: "100%", minHeight: "100vh", margin: 0, padding: 0 }}
    >
      {/* Sidebar Navigation */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-outline-variant flex flex-col transition-transform duration-300 ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
      >
        <div className="p-lg flex items-center justify-between lg:justify-start gap-sm">
          ...
          <button
            className="lg:hidden cursor-pointer p-2 hover:bg-surface-container rounded-lg"
        ...
        <nav className="flex-1 px-md space-y-1 py-md">
          <a
            className={`flex items-center gap-md px-md py-3 rounded-xl font-semibold transition-all group no-underline ${activeMenu === "Tournaments" ? "active-nav" : "text-on-surface-variant hover:bg-surface-container"}`}
            onClick={() => {
          ...
          <a
            className="flex items-center gap-md px-md py-3 rounded-xl text-on-surface-variant hover:bg-surface-container transition-all group no-underline"
            href="#"
        ...
        <div className="p-md mt-auto border-t border-outline-variant">
          <button
            className="w-full flex items-center gap-md px-md py-3 text-error hover:bg-error-container/20 rounded-xl transition-all"
          ...
          {currentSubView === "create" ? (
            ...
            <div className="w-full">
              ...
              <div className="bg-white rounded-xl border border-outline-variant p-6 shadow-sm opacity-60 pointer-events-none">
                <form>
                  ...
                  <button
                    type="button"
                    className="bg-primary text-white px-4 py-2 rounded-md font-bold w-full mt-4"
                    disabled
                  >
                    SAVE TOURNAMENT
                  </button>
            ...
            <>
              ...
              <div className="flex-1 flex flex-col min-w-0">
                <div
                  className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-lg gap-2"
              ...
              <div className="w-full xl:w-[400px] flex flex-col gap-lg">
                {selectedTourney ? (
                  <div className="bg-white rounded-xl border border-outline-variant shadow-sm overflow-hidden flex flex-col h-full">
                    <div
                      className="p-lg bg-surface-container-low border-b border-outline-variant"
                    ...
                    >
                      ...
                      >
                        <div className="flex justify-between">
                          ...
                          {selectedTourney.status === "PUBLISHED" ||
                          selectedTourney.status === "Published" ? (
                            <span
                              className="text-success font-bold text-xs uppercase"
                              style={{ fontSize: "12px" }}
                            >
                              Published
                            </span>
                            ...
                            selectedTourney.status === "Cancelled" ? (
                            <span
                              className="text-error font-bold text-xs uppercase"
                              style={{ fontSize: "12px" }}
                            >
                              Cancelled
                            </span>
                            ...
                            <span
                              className="text-on-surface font-bold text-xs uppercase"
                              style={{ fontSize: "12px" }}
                            >
                              Draft
                            </span>
                    ...
                    >
                      <div className="flex items-center justify-between mb-md">
                        <h4
                          className="font-label-caps text-on-surface-variant text-[11px] tracking-widest uppercase"
                          style={{
                            fontSize: "11px",
                            fontWeight: 700,
                            margin: 0,
                        ...
                        <button
                          className="flex items-center gap-1 text-primary text-xs font-bold hover:bg-primary/5 px-2 py-1 rounded transition-colors border-0 bg-transparent"
                      ...
                      >
                        {tournamentRaces.map((race, index) => (
                          <div
                            key={race.id || index}
                            className="p-md rounded-xl border border-outline-variant/30 bg-surface-container-lowest hover:border-primary/50 transition-all cursor-pointer group"
                            style={{
                              padding: "1rem",
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                {race.name || race.raceName || "Unnamed Race"}
                              </span>
                              <span className="text-[10px] px-2 py-0.5 bg-primary/5 text-primary rounded font-bold">
                                {race.code || `RACE ${index + 1}`}
                              </span>
                              {(race.status === "PUBLISHED" ||
                                race.status === "Published") && (
                                <span className="px-2 py-0.5 bg-success/10 text-success rounded text-[10px] font-bold uppercase ml-2">
                                  published
                                </span>
                              )}
                              {(race.status === "CANCELLED" ||
                                race.status === "Cancelled") && (
                                <span className="px-2 py-0.5 bg-error-container text-on-error-container rounded text-[10px] font-bold uppercase ml-2">
                                  cancelled
                                </span>
                              )}
                              {(race.status === "PENDING_REFEREE" ||
                                race.status === "Pending_Referee") && (
                                <span className="px-2 py-0.5 bg-surface-container text-on-surface-variant rounded text-[10px] font-bold uppercase ml-2">
                                  pending_referee
                                </span>
                              )}
                            </div>
                            <div className="flex items-center justify-between">
                              ...
                              <button
                                className="flex items-center gap-1 px-3 py-1 rounded border border-primary/30 text-primary text-[11px] font-bold hover:bg-primary/5 transition-colors bg-transparent"
                        ...
                        {tournamentRaces.length === 0 && (
                          <div
                            className="text-center py-5 text-muted border border-dashed rounded-3 mt-4"
                  ...
                  <div
                    className="card border-0 shadow-sm rounded-xl bg-white h-full p-4 d-flex align-items-center justify-content-center"
                    style={{
                      display: "flex",
                      alignItems: "center",
                  ...
                  >
                    <span className="text-muted text-on-surface-variant">
      ...
      {notification && (
        <div
          className={`fixed bottom-4 right-4 z-[100] px-6 py-4 rounded-xl shadow-2xl text-sm font-bold border transition-all ${
