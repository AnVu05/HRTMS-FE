...
const MOCK_JOCKEYS = [
  { id: '1', name: 'Nguyễn Văn Minh', nationality: 'Vietnam 🇻🇳', license: 'VN001', status: 'Active', winRate: 68, earnings: 340000, races: 124, avgPosition: 2.3, bio: 'A highly experienced local talent with a knack for dirt tracks. Known for explosive final stretches and tactical positioning.' },
  { id: '2', name: "James O'Brien", nationality: 'Ireland 🇮🇪', license: 'IE042', status: 'Active', winRate: 71, earnings: 520000, races: 215, avgPosition: 1.8, bio: 'International star who recently joined the Asian circuit. Exceptional handling of temperamental horses.' },
  { id: '3', name: 'Takeshi Yamamoto', nationality: 'Japan 🇯🇵', license: 'JP019', status: 'Active', winRate: 55, earnings: 280000, races: 86, avgPosition: 3.1, bio: 'Emerging talent from the Japanese racing academies. Specializes in long-distance endurance races.' },
];
...
export default function PortalJockeyProfile() {
  ...
  // Find jockey or default to first for demonstration
  const jockey = MOCK_JOCKEYS.find(j => j.id === id) || MOCK_JOCKEYS[0];
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <p className="text-xl text-slate-400 mb-6 flex items-center justify-center md:justify-start gap-2">
                <MapPin className="h-5 w-5" /> {jockey.nationality.split(' ')[0]}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 bg-slate-900/50 p-6 rounded-xl border border-slate-800 inline-flex text-left w-full max-w-3xl">
                ...
                <div>
                  <div className="text-slate-400 text-sm font-medium mb-1 flex items-center gap-2">
                    <Trophy className="h-4 w-4 text-primary" /> Win Rate
                  </div>
                  <div className="text-2xl font-bold">{jockey.winRate}%</div>
                </div>
                <div>
                  <div className="text-slate-400 text-sm font-medium mb-1 flex items-center gap-2">
                    <DollarSign className="h-4 w-4 text-primary" /> Earnings
                  </div>
                  <div className="text-2xl font-bold text-green-400">${jockey.earnings.toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-slate-400 text-sm font-medium mb-1 flex items-center gap-2">
      ...
      <div className="container mx-auto px-4 md:px-6 -mt-10 relative z-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            ...
            <Card className="bg-white shadow-sm border-slate-200">
              <CardHeader>
                <CardTitle className="text-lg">Frequent Horses</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {['Thunderbolt', 'Crimson Sky', 'River Spirit'].map((horse, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 bg-slate-50 hover:bg-slate-100 transition-colors">
                    <div className="font-medium text-slate-800">{horse}</div>
                    <div className="text-xs font-semibold text-slate-500 bg-white px-2 py-1 rounded border shadow-sm">
                      {5 - idx} Wins
                    </div>
                  </div>
                ))}
